...
const fish = async function(userid, messageTime) {
  const end = [];
  ...
  for (const item of items) {
    if (item > 0.95) {
      const result = dataManager.items.filter(i => i.rarity === 1 && i.source === 'fishing').random();
    ...
    if (item > 0.05) {
      const result = dataManager.items.filter(i => i.rarity === 2 && i.source === 'fishing').random();
      const amt = Math.floor(Math.random() * 2 + 1);
      if (end.indexOf(result.name) === -1) {
        end.push(result.name, amt, result.id);
      }
