        ...
        if (args.add && !args.del && !args.reset) {
            if (args.prefix.length < 3) {
                try {
                    ...
                    console.log(this.client.guildSettings.get(message.guild.id).prefix)
                    await this.client.db.query("UPDATE guildsettings SET prefix = $1 WHERE guildid = $2", [this.client.guildSettings.get(message.guild.id).prefix, message.guild.id])
