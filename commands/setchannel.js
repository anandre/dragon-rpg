    ...
    async exec(message, args) {
        ...
        if (args.add && !args.del && !args.reset && args.channel) {
            try {
                ...
                console.log(this.client.guildSettings.get(message.guild.id).channel)
                await this.client.db.query("UPDATE guildsettings SET channel = $1 WHERE guildid = $2", [this.client.guildSettings.get(message.guild.id).channel, message.guild.id]);
