...
class HuntCommand extends Command {
  ...
  async exec(message) {
    ...
    if (parseInt(lastHunt.rows[0].hunttimer) + timer < message.createdTimestamp) {
      const gold = await dataManager.functions.gold(message.author.id);
      const hunt = await dataManager.functions.hunt(message.author.id, message.createdTimestamp, gold);
      let huntResult = '';
      for (let i = 0; i < hunt.length; i += 3) {
      ...
      const cdtime = Math.abs(message.createdTimestamp - (parseInt(lastHunt.rows[0].hunttimer) + timer));
      return message.answer(message.author, `you have hunted too recently.  You may hunt again in ${dataManager.functions.time(cdtime)}.`);
    }
  }
}
