...
class GatherCommand extends Command {
  ...
  async exec(message) {
    const timer = 1800000;
    const lastGather = await this.client.db.query(`
      SELECT
        gathertimer
      FROM
        players
    ...
    if (parseInt(lastGather.rows[0].gathertimer) + timer < message.createdTimestamp) {
      ...
      return message.answer(message.author, `you have gathered too recently.  You may gather again in ${this.client.execute(cdtime)}.`)
    }
  }
}
