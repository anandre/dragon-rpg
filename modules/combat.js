module.exports = (client) => {
    ...
    client.turnIncrement = async (player, enemy) => {
        ...
        for (let k = 0; k < player.enemycd.length; k++) {
            if (player.enemycd[i] !== client.abilities.get(enemy.abilities[i]).cooldown) {
                player.enemycd[i]--;
            };
            if (player.enemycd[i] < 0) {
                player.enemycd[i] = client.abilities.get(enemy.abilties[i].cooldown)
            };
        };
        if (player.turn % 3 === 0) {
